...
1. 在「配置」卡片填入 **API ID / API Hash**、**源频道**（每行一个）、**目标频道**、搬运模式等，点击「保存配置」。
2. 在「账号登录」卡片填手机号 → 点「发送验证码」→ 输入 Telegram 收到的验证码（如开了两步验证再填密码）→ 点「登录」。
3. 回到「运行状态」卡片，点「▶ 启动监控」。之后源频道一发新视频就会自动搬运。
4. 「运行日志」会实时刷新；随时可点「■ 停止」。
