...
- ⚖️ **账号负载均衡组**：把多个账号编入一个组共同搬运，发送任务在组内账号间轮流分摊，并感知每账号节流与限流冷却，从而规避单账号被限速。
- 🧹 **内容过滤**：搬运时可屏蔽指定文字关键词、移除链接（http/t.me/裸域名）与 @提及；命中关键词可选「删除整行」或「丢弃整条」。
- 🎬 **视频处理**：上传前可删除视频开头的若干秒（如开头 10 秒广告）；搬运相册(视频+图片+文字)时可用相册里的图片作为视频封面。需安装 ffmpeg。
...
当单个账号搬运量大、容易触发 Telegram 限流（FloodWait）时，可用「负载均衡组」把发送压力分摊到多个账号：

1. 先添加并**登录好若干账号**（这些账号都需要能访问同一批源频道与目标频道）。
2. 点左侧「＋ 新建均衡组」，选中该组后在「组配置」里：
   - 勾选要参与分摊的**成员账号**；
   - 填**源频道 / 目标频道**、搬运模式；
   - 设**每账号发送间隔秒数**（节流，越大越不易限速）与**调度策略**；
   - 点「保存组配置」。
3. 点「▶ 启动组」。

工作方式：

- 组内选一个账号当**监听者**监听源频道；发现新视频后进入**分发队列**。
- 调度器把每条视频的**发送任务**交给当前「最早可用」的成员账号执行（`balanced` 策略），或严格轮转（`round_robin`）。
- 每个账号发送后会按「每账号发送间隔」进入短暂节流；某账号触发限流时会被标记**冷却**，自动跳过并改用其它账号，冷却结束后再纳入轮转。
- 由此，总发送量被摊到多个账号，单账号的发送频率显著下降，从而规避限速。

> 注意：同一账号若已编入某个运行中的组，就不要再单独启动它去监控相同源频道，否则会重复搬运。

### 内容过滤（屏蔽文字 / 链接 / @提及）🧹

在账号「配置」或组「组配置」里都有「内容过滤」一栏，作用于视频的文案(caption)：

- **屏蔽关键词**（每行一个）：
  - 默认行为：删除文案中**包含该关键词的整行**；
  - 勾选「命中屏蔽关键词时丢弃整条」后：只要命中关键词，**整条视频都不搬运**。
- **移除链接**：去掉文案里的 `http(s)://`、`t.me/`、以及常见后缀的裸域名（如 `xxx.com`）。
- **移除 @提及**：去掉 `@用户名`（3 位及以上）。
- **替换文本**：被移除的链接/@ 用它替换，留空则直接删除。

> 说明：文案改写仅在 `copy` 模式下生效（`forward` 模式无法修改原消息）；但「丢弃整条」在两种模式下都有效。关键词匹配不区分大小写。

### 视频处理：裁掉开头 + 相册封面 🎬

在账号「配置」或组「组配置」里的「视频处理」一栏（仅 `copy` 模式生效）：

- **删除视频开头秒数**：填 `10` 即上传前把视频开头 10 秒删掉（常用于去掉片头广告）。原理是下载视频后用 ffmpeg 裁剪再上传。
- **相册用图片当封面**：当搬运的是一条相册消息（视频 + 图片 + 文字）时，下载相册里的图片作为视频的封面缩略图；文案优先取相册里的文字。

**依赖 ffmpeg**：视频裁剪需要服务器安装 `ffmpeg` 与 `ffprobe`。
...
未安装 ffmpeg 时，会自动跳过裁剪、按原视频上传（不影响其它功能）。可用环境变量 `FFMPEG_BIN` / `FFPROBE_BIN` 指定可执行文件路径。

> 提示：裁剪会先尝试无损「流复制」，速度快；若开头异常则自动回退为重新编码（较慢）。处理大视频较耗时与占带宽，请按需开启。
...
├── media.py           # 视频处理辅助（ffmpeg 裁剪 / ffprobe 探测）
